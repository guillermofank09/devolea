...
export const createProfesor = async (req: Request, res: Response) => {
  ...
  try {
    const profesor = await getService().create({ name, phone, hourlyRate: hourlyRate != null ? Number(hourlyRate) : undefined }, userId);
...
export const updateProfesor = async (req: Request, res: Response) => {
  const { name, phone, hourlyRate } = req.body;
  try {
    const profesor = await getService().update(Number(req.params.id), { name, phone, hourlyRate: hourlyRate != null ? Number(hourlyRate) : undefined });
    if (!profesor) { res.status(404).json({ error: "Profesor no encontrado" }); return; }
    res.json(profesor);
  } catch {
