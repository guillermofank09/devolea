...
export const createProfesor = async (req: Request, res: Response) => {
  const { name, phone, hourlyRate, sex, schedule, avatarUrl, city, sport, sports, birthDate } = req.body;
  ...
  try {
    const profesor = await getService().create({ name, phone, hourlyRate: hourlyRate != null ? Number(hourlyRate) : undefined, sex, schedule, avatarUrl, city, sport: primarySport, sports: sportsArr, birthDate }, userId);
...
export const updateProfesor = async (req: Request, res: Response) => {
  ...
  const primarySport = sportsArr?.length ? sportsArr[0] : (sport ?? undefined);
  try {
    const profesor = await getService().update(Number(req.params.id), { name, phone, hourlyRate: hourlyRate != null ? Number(hourlyRate) : undefined, sex, schedule, avatarUrl, city, sport: primarySport, sports: sportsArr, birthDate });
    if (!profesor) { res.status(404).json({ error: "Profesor no encontrado" }); return; }
    res.json(profesor);
  } catch {
