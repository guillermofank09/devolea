...
export const createProfesor = async (req: Request, res: Response) => {
  const { name, phone, hourlyRate, sex, schedule, avatarUrl, sport } = req.body;
  if (!name) { res.status(400).json({ error: "El nombre es requerido" }); return; }
  const userId = req.authUser!.sub;
  try {
    const profesor = await getService().create({ name, phone, hourlyRate: hourlyRate != null ? Number(hourlyRate) : undefined, sex, schedule, avatarUrl, sport }, userId);
...
export const updateProfesor = async (req: Request, res: Response) => {
  const { name, phone, hourlyRate, sex, schedule, avatarUrl, sport } = req.body;
  try {
    const profesor = await getService().update(Number(req.params.id), { name, phone, hourlyRate: hourlyRate != null ? Number(hourlyRate) : undefined, sex, schedule, avatarUrl, sport });
    if (!profesor) { res.status(404).json({ error: "Profesor no encontrado" }); return; }
    res.json(profesor);
  } catch {
